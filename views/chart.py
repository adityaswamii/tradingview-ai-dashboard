...
df['Support'] = df['Support'].apply(lambda x: x if x else np.nan)
df['Support'] = df['Support'].ffill().bfill()
df['Resistance'] = df['Resistance'].apply(lambda x: x if x else np.nan)
...
# Build the support and resistance band lines
